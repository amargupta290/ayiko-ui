...
import {customerOrders} from 'store/slices/DriverSlice';

const TrackOrderList = ({navigation, route}: {navigation: any; route: any}) => {
  ...
  const {customerOrdersData} = useAppSelector(
  ...
  console.log(
    'customerOrdersData',
    JSON.stringify(customerOrdersData, null, 2),
  );
  ...
  const renderItem = ({item, index}: any) => {
    return (
      <TouchableOpacity
        style={[
          styles.card,
          {marginBottom: customerOrdersData?.length == index + 1 ? 150 : 8},
        ]}
        onPress={() => navigation.navigate('TrackScreen', {id: item?.id})}
        key={index}>
        <ImageComp
          source={{
            uri: item?.cart?.supplier?.businessImages
              ? item?.cart?.supplier?.businessImages[0]
              : '',
          }}
          imageStyle={styles.image}
        />
        <View style={styles.cardDescription}>
          <Text style={styles.title}>{item?.cart?.supplier?.businessName}</Text>
          <Text
            style={{
              ...styles.unitPrice,
              color: globalHelpers
                ?.getCustomerOrderStatus(item)
                ?.toLowerCase()
                .includes(['pending'])
                ? 'green'
                : 'green',
            }}>
            {globalHelpers?.getCustomerOrderStatus(item)}
          </Text>
      ...
      <View
        ...
        <View style={styles.header}>
          ...
          {/* <View style={styles.searchWrapper}>
            <SVGSearch />
            <TextInput
              placeholder="Search for items or customerOrdersData"
